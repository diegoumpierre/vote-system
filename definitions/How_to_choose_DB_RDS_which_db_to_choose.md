...
- Low-latency queries
- Strong consistency
- Typical for APIs, e-commerce, logistics, payments, CRUD systems
...
- Scans across millions/billions of rows
- Historical data analysis
- Dashboarding, BI, reporting
- Append-driven pipelines (few updates)
...
- Ideal for point lookups and retrieving full rows
- Strong ACID support (in most engines)

Cons:
- Slow for analytics requiring full-table scans
...
- High compression → lower storage cost
- Ideal for large dashboards and BI queries

Cons:
- Slower for frequent updates and row-level transactions
- Not built for OLTP workloads
...
- Any system where correctness > performance
...
Use BASE when:
- Data can be slightly out-of-sync
...
-  how data must replicate
- how global the system is
...
- RTO (Recovery Time Objective)
	How fast the system must recover after failure.
	- Aurora RTO ~ seconds
	- PostgreSQL RTO depends on failover/pooling
	- Redshift RTO is much slower

- RPO (Recovery Point Objective)
	How much data the system can lose.
	- Multi-AZ → 0
