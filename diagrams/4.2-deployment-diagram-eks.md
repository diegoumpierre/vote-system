...
> **Note:** This is an alternative version using **EKS (Elastic Kubernetes Service)** instead of ECS on EC2. See [4.2-deployment-diagram.md](4.2-deployment-diagram.md) for the ECS on EC2 version.
...
- **API Gateway**: Throttling, JWT validation via native JWT Authorizer (Auth0 issuer + audience)
...
| **Scaling** | EC2 ASG + ECS Service Auto Scaling | Pod (HPA) + Node (Cluster Autoscaler) |
| **Ecosystem** | AWS tools | Rich K8s ecosystem (Helm, Operators, etc.) |
| **Cost** | EC2 instances only (Reserved/Spot available) | EC2 nodes + EKS control plane fee ($0.10/hr) |
| **Cold Start** | Zero (instances pre-provisioned) | Zero if nodes pre-provisioned |
| **Best For** | AWS-focused teams, simpler ops | Teams with K8s experience, multi-cloud needs |
